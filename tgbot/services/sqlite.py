...
def register_user(id, user_name, first_name, language='ru'):
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO users("
                    "id, is_ban, user_name, first_name, balance, count_refills, reg_date, reg_date_unix, language) "
                    "VALUES (?,?,?,?,?,?,?, ?, ?)", [id, "False", user_name, first_name, 0, 0, get_date(), get_unix(), language])
        con.commit()
...
# Обновление баланса пользователя с учетом пополнения
def update_user_balance_and_refill(user_id, amount):
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        # Получаем текущие значения
        user = con.execute("SELECT balance, total_refill FROM users WHERE id = ?", [user_id]).fetchone()
        if user:
            new_balance = user['balance'] + amount
            new_total_refill = user['total_refill'] + amount
            # Обновляем оба поля
            con.execute("UPDATE users SET balance = ?, total_refill = ? WHERE id = ?", 
                       [new_balance, new_total_refill, user_id])
            con.commit()
            return new_balance
        return None
...
def create_db():
    with sqlite3.connect(path_db) as con:
        ...
        # Платежные системы
        if len(con.execute("PRAGMA table_info(payments)").fetchall()) == 4:
            ...
            con.execute("INSERT INTO payments("
                        "pay_crystal, pay_yoomoney, pay_lolz, pay_lava) "
                        "VALUES (?, ?, ?, ?)",
                        ['False', 'False', 'False', 'False'])
            print("database was recreated without qiwi (payment systems| 3/11), creating...")
        ...
        if len(con.execute("PRAGMA table_info(categories)").fetchall()) == 3:
            ...
            con.execute("INSERT INTO categories (id, name) VALUES (?, ?)", (1, "Товары"))
        ...
        if len(con.execute("PRAGMA table_info(purchases)").fetchall()) == 12:
            print("database was found (Purchaches | 11/12)")
        else:
            con.execute("CREATE TABLE purchases("
                        ...
                        "date TIMESTAMP,"
                        "unix INTEGER)")
            print("database was not found (Purchaches | 11/12), creating...")

        # Использованные транзакции
        if len(con.execute("PRAGMA table_info(used_transactions)").fetchall()) == 6:
            print("database was found (Used Transactions | 12/12)")
        else:
            con.execute("CREATE TABLE used_transactions("
                        "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "tx_hash TEXT UNIQUE,"
                        "user_id INTEGER,"
                        "amount REAL,"
                        "date TIMESTAMP,"
                        "date_unix INTEGER)")
            print("database was not found (Used Transactions | 12/12), creating...")

            con.commit()

# Упрощенные функции для работы с товарами без категорий
def add_position_simple(name, description, price, infinity=False):
    """Добавление позиции без категории"""
    pos_id = random.randint(1000000000, 9999999999)
    add_position(name, price, description, "-", 1, "+" if infinity else "-", pos_id, None)
    return pos_id

def get_positions_simple():
    """Получение всех позиций"""
    return get_all_positions()

def delete_position_simple(pos_id):
    """Удаление позиции"""
    del_position(pos_id)

def add_item_simple(position_id, item_data):
    """Добавление одного товара к позиции"""
    add_item(1, position_id, [item_data])

def get_items_simple(position_id):
    """Получение товаров позиции"""
    return get_items(position_id=position_id)

def delete_item_simple(item_id):
    """Удаление товара"""
    remove_item(id=item_id)

def update_position_simple(pos_id, **kwargs):
    """Обновление позиции"""
    update_position(pos_id, **kwargs)

# Функции для работы с использованными транзакциями
def add_used_transaction_db(tx_hash, user_id, amount):
    """Добавление использованной транзакции в БД"""
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO used_transactions (tx_hash, user_id, amount, date, date_unix) VALUES (?, ?, ?, ?, ?)",
                   [tx_hash, user_id, amount, get_date(), get_unix()])
        con.commit()

def is_transaction_used_db(tx_hash):
    """Проверка использованности транзакции в БД"""
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        result = con.execute("SELECT * FROM used_transactions WHERE tx_hash = ?", (tx_hash,)).fetchone()
        return result is not None

def get_all_used_transactions():
    """Получение всех использованных транзакций"""
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        return con.execute("SELECT * FROM used_transactions").fetchall()
...
# Получение покупки по чеку
def get_purchase_by_receipt(receipt):
    """Получение покупки по номеру чека"""
    with sqlite3.connect(path_db) as con:
        cur = con.cursor()
        cur.execute("SELECT * FROM purchases WHERE receipt = ?", (receipt,))
        return dict_factory(cur, cur.fetchone())
...
# Получение всех покупок (для статистики)
def all_purchases():
    """Получение всех покупок"""
    with sqlite3.connect(path_db) as con:
        cur = con.cursor()
